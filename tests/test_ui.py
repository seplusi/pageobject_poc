...
def test_convert_real2euros(webdriver, config):
    ...
    assert home_page.conversion_1st_line() == f'100.00 Brazilian Real ='
    home_page.conversion_result(100, 'Euro')
...
def test_webview_mobile_home_page(config, chrome_appiumriver):
    ...
    assert home_page.conversion_text1 == "100.00 Brazilian Real ="
...
def test_native_app_calc(config, calc_appiumdriver):
    ...
    assert home_page.result_typed.text == '3 Calculation result'
    assert home_page.result_tab.text == ''
